...
function initCountdown() {
    ...
    startCountdown(WEDDING_CONFIG.date, (result) => {
        ...
        if (daysLabel)
            daysLabel.textContent = pluralize(result.days, 'день', 'дня', 'дней');
        if (hoursLabel)
            hoursLabel.textContent = pluralize(result.hours, 'час', 'часа', 'часов');
        if (minutesLabel)
            minutesLabel.textContent = pluralize(result.minutes, 'минута', 'минуты', 'минут');
        if (secondsLabel)
            secondsLabel.textContent = pluralize(result.seconds, 'секунда', 'секунды', 'секунд');
    });
}
// ─── Расписание ───────────────────────────────────────────────────────────────
...
function renderSaveDateCalendar() {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (mapLink)
        mapLink.href = WEDDING_CONFIG.venue.mapUrl;
    const addressEl = document.getElementById('venue-address');
    if (addressEl)
        addressEl.textContent = WEDDING_CONFIG.venue.address;
    // Запускаем модули
    initCountdown();
    renderSchedule();
